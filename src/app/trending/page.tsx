...
export default async function TrendingPage() {
  try {
    ...
    if (!trendingCoins || trendingCoins.length === 0) {
      return (
        <ApiErrorDisplay 
          ...
          message="Trend coin verisi bulunamadı. Lütfen daha sonra tekrar deneyin."
          backLink="/"
          ...
          <div className="divide-y divide-gray-700">
            {trendingCoins.map((coin) => (
              <Link 
                ...
                key={coin.coin} 
                className="grid grid-cols-6 gap-4 p-4 hover:bg-[#222] block"
              >
                <div className="flex items-center gap-2">
                  <div>
                    <div className="font-medium">{coin.coinMetadata.name || 'İsimsiz Coin'}</div>
    ...
    console.error('Trend coinler sayfası yüklenirken hata:', error);
    return (
      <ApiErrorDisplay 
        title="Trend coinler yüklenemedi" 
        message="Trend coin verileri getirilirken bir hata oluştu. Lütfen daha sonra tekrar deneyin."
        backLink="/"
        backText="Ana Sayfaya Dön"
      />
    );
